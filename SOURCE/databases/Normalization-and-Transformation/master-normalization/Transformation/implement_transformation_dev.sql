...
-- Create Necessary tables
	CREATE TABLE `tngdb_tempo`.`CupContentAuthor` LIKE `tngdb_prod`.`CupContentAuthor`;
	CREATE TABLE `tngdb_tempo`.`CupContentFormat` LIKE `tngdb_prod`.`CupContentFormat`;
	CREATE TABLE `tngdb_tempo`.`CupContentSeries` LIKE `tngdb_prod`.`CupContentSeries`;
	CREATE TABLE `tngdb_tempo`.`CupContentSeriesFormats` LIKE `tngdb_prod`.`CupContentSeriesFormats`;
	CREATE TABLE `tngdb_tempo`.`CupContentSeriesSubjects` LIKE `tngdb_prod`.`CupContentSeriesSubjects`;
	CREATE TABLE `tngdb_tempo`.`CupContentSubject` LIKE `tngdb_prod`.`CupContentSubject`;
    CREATE TABLE `tngdb_tempo`.`CupGoFolderContent` LIKE `tngdb_prod`.`CupGoFolderContent`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitle` LIKE `tngdb_prod`.`CupContentTitle`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitleAuthors` LIKE `tngdb_prod`.`CupContentTitleAuthors`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitleSamplePages` LIKE `tngdb_prod`.`CupContentTitleSamplePages`;
    CREATE TABLE `tngdb_tempo`.`CupContentTitleFormats` LIKE `tngdb_prod`.`CupContentTitleFormats`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitleSubjects` LIKE `tngdb_prod`.`CupContentTitleSubjects`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitleSupportingTitle` LIKE `tngdb_prod`.`CupContentTitleSupportingTitle`;
	CREATE TABLE `tngdb_tempo`.`CupContentTitleRelatedTitle` LIKE `tngdb_prod`.`CupContentTitleRelatedTitle`;
	CREATE TABLE `tngdb_tempo`.`CupGoContentFolders` LIKE `tngdb_prod`.`CupGoContentFolders`;
	CREATE TABLE `tngdb_tempo`.`CupGoContent` LIKE `tngdb_prod`.`CupGoContent`;
	CREATE TABLE `tngdb_tempo`.`CupGoContentDetail` LIKE `tngdb_prod`.`CupGoContentDetail`;
    CREATE TABLE `tngdb_tempo`.`CupGoTabHmIds` LIKE `tngdb_prod`.`CupGoTabHmIds`;
	CREATE TABLE `tngdb_tempo`.`CupGoTabs` LIKE `tngdb_prod`.`CupGoTabs`;
	CREATE TABLE `tngdb_tempo`.`CupGoTabContent` LIKE `tngdb_prod`.`CupGoTabContent`;
    CREATE TABLE `tngdb_tempo`.`CupGoBrandCodeTitles` LIKE `tngdb_prod`.`CupGoBrandCodeTitles`;
...
-- Apply Triggers to temporary database (cap_triggers.sql -- make sure this is updated and pointing to correct master db)
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentFormat`
	FOR EACH ROW 
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentFormat`
	FOR EACH ROW 
	  UPDATE `cap`.`formats` 
	  SET
	  id=NEW.id,
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentAuthor`
	FOR EACH ROW 
	  UPDATE `cap`.`authors` 
	  SET
	  id=NEW.id,
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentAuthor`
	FOR EACH ROW 
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentSubject`
	FOR EACH ROW 
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentSubject`
	FOR EACH ROW 
	BEGIN
	  DECLARE var_country_id INT(11);
	  SELECT id INTO var_country_id FROM `cap`.`countries` WHERE iso_code = NEW.region COLLATE utf8_unicode_ci;
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentSubject`
	FOR EACH ROW 
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentSeries`
	FOR EACH ROW 
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentSeries`
	FOR EACH ROW 
	BEGIN
	  UPDATE `cap`.`series` 
	  SET
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentSeries`
	FOR EACH ROW 
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentSeriesFormats`
	FOR EACH ROW 
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentSeriesFormats`
	FOR EACH ROW 
	BEGIN
	  DECLARE var_format_id INT(11);
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentSeriesSubjects`
	FOR EACH ROW 
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentSeriesSubjects`
	FOR EACH ROW 
	BEGIN
	  DECLARE var_subject_id INT(11);
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitle`
	FOR EACH ROW 
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentTitle`
	FOR EACH ROW
	BEGIN
	  DECLARE var_series_id INT(11);
	  DECLARE var_edition_id INT(11);
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitle`
	FOR EACH ROW
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoBrandCodeTitles`
	FOR EACH ROW
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoBrandCodeTitles`
	FOR EACH ROW
	BEGIN
	  DECLARE var_brand_code_id INT(11);
	  SELECT id INTO var_brand_code_id FROM `cap`.`brand_codes` WHERE LOWER(`name`) = LOWER(NEW.brandCode);
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoBrandCodeTitles`
	FOR EACH ROW
	BEGIN
	  DECLARE var_brand_code_id INT(11);
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleAuthors`
	FOR EACH ROW
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleAuthors`
	FOR EACH ROW
	BEGIN
	  DECLARE var_author_id INT(11);
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleFormats`
	FOR EACH ROW
	BEGIN
	  DECLARE var_format_id INT(11);
	  SELECT id INTO var_format_id FROM `cap`.`formats` WHERE name = NEW.format COLLATE utf8_unicode_ci;
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleFormats`
	FOR EACH ROW
	BEGIN
	  DECLARE var_format_id INT(11);
	  SELECT id INTO var_format_id FROM `cap`.`formats` WHERE name = OLD.format COLLATE utf8_unicode_ci;
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleRelatedTitle`
	FOR EACH ROW
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleRelatedTitle`
	FOR EACH ROW
	BEGIN
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleSamplePages`
	FOR EACH ROW
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupContentTitleSamplePages`
	FOR EACH ROW
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleSamplePages`
	FOR EACH ROW
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleSubjects`
	FOR EACH ROW
	BEGIN
	  DECLARE var_subject_id INT(11);
	  SELECT id INTO var_subject_id FROM `cap`.`subjects` WHERE name = NEW.subject COLLATE utf8_unicode_ci;
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleSubjects`
	FOR EACH ROW
	BEGIN
	  DECLARE var_subject_id INT(11);
	  SELECT id INTO var_subject_id FROM `cap`.`subjects` WHERE name = OLD.subject COLLATE utf8_unicode_ci;
	...
	AFTER INSERT ON `tngdb_tempo`.`CupContentTitleSupportingTitle`
	FOR EACH ROW
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupContentTitleSupportingTitle`
	FOR EACH ROW
	BEGIN
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoContentFolders`
	FOR EACH ROW
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoContentFolders`
	FOR EACH ROW
	BEGIN
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoContentFolders`
	FOR EACH ROW
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoTabs`
	FOR EACH ROW
	BEGIN
	  DECLARE var_tab_access_id INT(11);
	  ...
	  DECLARE var_is_content_visibility_open INT(11);
		...
		SELECT id INTO var_tab_access_id FROM `cap`.`tab_accesses` WHERE name = NEW.ContentAccess;
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoTabs`
	FOR EACH ROW
	BEGIN
	  DECLARE var_tab_access_id INT(11);
	  DECLARE var_is_public VARCHAR(255);
	  ...
	  SET
		id=NEW.id,
		title_id=NEW.TitleID,
		tab_access_id=var_tab_access_id,
		name=var_tab_name,
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoTabs`
	FOR EACH ROW
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoTabContent`
	FOR EACH ROW
	BEGIN
	  DECLARE var_is_active INT(11);
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoTabContent`
	FOR EACH ROW
	BEGIN
	  DECLARE var_is_active INT(11);
		DECLARE var_is_demo_only INT(11);
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoTabContent`
	FOR EACH ROW
	  DELETE FROM `cap`.`tab_contents` WHERE id = OLD.ID;


	CREATE TRIGGER tab_hm_ids_after_insert 
	AFTER INSERT ON `tngdb_tempo`.`CupGoTabHmIds`
	FOR EACH ROW 
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoTabHmIds`
	FOR EACH ROW 
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoContent`
	FOR EACH ROW
	BEGIN
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoContent`
	FOR EACH ROW
	BEGIN
	  DECLARE var_is_global INT(11);
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoContent`
	FOR EACH ROW
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoFolderContent`
	FOR EACH ROW
	BEGIN
	  UPDATE `cap`.`contents`
	...
	AFTER INSERT ON `tngdb_tempo`.`CupGoContentDetail`
	FOR EACH ROW
	BEGIN
	  DECLARE var_is_active INT(11);
		DECLARE var_is_visible INT(11);
	...
	AFTER UPDATE ON `tngdb_tempo`.`CupGoContentDetail`
	FOR EACH ROW
	BEGIN
	  DECLARE var_is_active INT(11);
		DECLARE var_is_visible INT(11);
	...
	AFTER DELETE ON `tngdb_tempo`.`CupGoContentDetail`
	FOR EACH ROW
...
-- Insert data so the trigger will be populating CAP data
	...
	-- INSERT INTO `tngdb_tempo`.`CupContentAuthor` SELECT * FROM `tngdb_prod`.`CupContentAuthor`;
	-- Insert the authors that are not in author look up
	-- INSERT INTO `tngdb_tempo`.`CupContentAuthor` (name, prettyUrl, createdAt, modifiedAt) SELECT DISTINCT(author), 'Unverified-Pretty-Url', NOW(), NOW() FROM `tngdb_prod`.`CupContentTitleAuthors` WHERE author NOT IN (SELECT name FROM `tngdb_prod`.`CupContentAuthor`);

	-- CupContentSubject (excluded since transformation was done in the initial transformation script)
	-- INSERT INTO `tngdb_tempo`.`CupContentSubject` SELECT * FROM `tngdb_prod`.`CupContentSubject`;
	-- Insert subjects that are not in the subject lookup
	-- INSERT INTO `tngdb_tempo`.`CupContentSubject` (name, prettyUrl, isPrimary, isSecondary, region, createdAt, modifiedAt) SELECT DISTINCT(subject), 'Unverified-Pretty-Url', 1, 1, 'ALL', NOW(), NOW() FROM `tngdb_prod`.`CupContentTitleSubjects` WHERE subject NOT IN (SELECT name FROM `tngdb_prod`.`CupContentSubject`);

	-- CupContentFormat (excluded since transformation was done in the initial transformation script)
	-- INSERT INTO `tngdb_tempo`.`CupContentFormat` SELECT * FROM `tngdb_prod`.`CupContentFormat`;

	-- CupContentSeries
	INSERT INTO `tngdb_tempo`.`CupContentSeries` SELECT * FROM `tngdb_prod`.`CupContentSeries`;

	-- CupContentSeriesFormats
	INSERT INTO `tngdb_tempo`.`CupContentSeriesFormats` SELECT * FROM `tngdb_prod`.`CupContentSeriesFormats`;
	...
	INSERT INTO `tngdb_tempo`.`CupContentSeriesSubjects` SELECT * FROM `tngdb_prod`.`CupContentSeriesSubjects`;
	...
	-- CupContentTitle
	INSERT INTO `tngdb_tempo`.`CupContentTitle` SELECT * FROM `tngdb_prod`.`CupContentTitle`;

	-- CupContentTitleAuthors
	INSERT INTO `tngdb_tempo`.`CupContentTitleAuthors` SELECT * FROM `tngdb_prod`.`CupContentTitleAuthors`;

	-- CupContentTitleSamplePages
	INSERT INTO `tngdb_tempo`.`CupContentTitleSamplePages` SELECT * FROM `tngdb_prod`.`CupContentTitleSamplePages`;

	-- CupContentTitleSubjects
	INSERT INTO `tngdb_tempo`.`CupContentTitleSubjects` SELECT * FROM `tngdb_prod`.`CupContentTitleSubjects`;

	-- CupContentTitleRelatedTitle
	INSERT INTO `tngdb_tempo`.`CupContentTitleRelatedTitle` SELECT * FROM `tngdb_prod`.`CupContentTitleRelatedTitle` cctrt WHERE cctrt.titleID IS NOT NULL AND cctrt.related_titleID IS NOT NULL;
	-- INSERT INTO `tngdb_tempo`.`CupContentTitleRelatedTitle` SELECT * FROM `tngdb_prod`.`CupContentTitleRelatedTitle` GROUP BY titleID, related_titleID, isbn13;

	-- CupContentTitleSupportingTitle
	INSERT INTO `tngdb_tempo`.`CupContentTitleSupportingTitle` SELECT * FROM `tngdb_prod`.`CupContentTitleSupportingTitle`;
    
    -- CupGoBrandCodeTitles
	INSERT INTO `tngdb_tempo`.`CupGoBrandCodeTitles` SELECT * FROM `tngdb_prod`.`CupGoBrandCodeTitles`;

	-- CupGoContentFolders
	INSERT INTO `tngdb_tempo`.`CupGoContentFolders` SELECT * FROM `tngdb_prod`.`CupGoContentFolders`;

	-- CupGoContent
	INSERT INTO `tngdb_tempo`.`CupGoContent` SELECT * FROM `tngdb_prod`.`CupGoContent`;

	-- CupGoContentDetail
	INSERT INTO `tngdb_tempo`.`CupGoContentDetail` SELECT * FROM `tngdb_prod`.`CupGoContentDetail`;

	-- CupGoTabs
	INSERT INTO `tngdb_tempo`.`CupGoTabs` SELECT * FROM `tngdb_prod`.`CupGoTabs`;

	-- CupGoTabContent
	INSERT INTO `tngdb_tempo`.`CupGoTabContent` SELECT * FROM `tngdb_prod`.`CupGoTabContent`;
    
    -- CupContentTitleFormats
	INSERT INTO `tngdb_tempo`.`CupContentTitleFormats` SELECT * FROM `tngdb_prod`.`CupContentTitleFormats`;
	SET FOREIGN_KEY_CHECKS = 1;
