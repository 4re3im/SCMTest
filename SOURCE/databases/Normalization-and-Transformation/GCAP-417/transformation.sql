...
INSERT INTO cap.content_access (name)
VALUES
('Free'),
('Login Only'),
('Subscription');
...
FROM `tngdb_prod`.`CupContentFormat`;


-- insert missing subjects first
INSERT INTO `tngdb_prod`.`CupContentSubject` (name, prettyUrl, isPrimary, isSecondary, description, region, createdAt, modifiedAt) SELECT DISTINCT(subject), 'Unverified-Pretty-Url', 1, 1, 'Description', 'ALL', NOW(), NOW() FROM `tngdb_prod`.`CupContentTitleSubjects` WHERE subject NOT IN (SELECT name FROM `tngdb_prod`.`CupContentSubject`);
...
FROM `tngdb_prod`.`CupContentSubject`;


-- insert missing authors first
INSERT INTO `tngdb_prod`.`CupContentAuthor` (name, prettyUrl, biography, createdAt, modifiedAt) SELECT DISTINCT(author), 'Unverified-Pretty-Url', 'Unknown Biography', NOW(), NOW() FROM `tngdb_prod`.`CupContentTitleAuthors` WHERE author NOT IN (SELECT name FROM `tngdb_prod`.`CupContentAuthor`);
...
FROM `tngdb_prod`.`CupContentAuthor`;
...
select * from cap.content_access;
select * from cap.types;
select * from cap.editions;
select * from cap.divisions;
select * from cap.formats;
select * from cap.subjects;
select * from cap.authors;