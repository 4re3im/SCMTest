 ...
 * Added to override dashboard header Return to Website Redirect.
 */
...
<script type="text/javascript">
	...
	// SB-252 added by mabrigos to return user back when clicking return to website
	var homeUrl =  window.location.protocol+'//'+window.location.hostname+'/go/';
	
	function redirectback(condition) {
		if (condition === 2) {
			history.go(-2)
		} else if (condition === 1) {
			history.back()
...
<li><a class="ccm-icon-back ccm-menu-icon" href="#" onclick="redirectback
<?php 
	// SB-252 added by mabrigos - condition to check referrer and session to return user.
	if ($_SESSION['redirectback'] && strpos($_SERVER['HTTP_REFERER'], 'login') !== false) { 
		echo "(2)"; 
	} else if ($_SESSION['redirectback']) { 
		echo "(1)"; 
	} else {
		echo "(0)";
	} ?> ">
<?php if ($md->isMobile()) { ?><?php echo t('Back')?><?php } else { ?><?php echo t('Return to Website')?><?php } ?></a></li>
