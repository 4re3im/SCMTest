...
$(document).ready(function () {
  ...
  $('.pending-notification button').click(function () {
      $('#pending-hm-wrapper-wrapper').fadeOut().css('display', 'none');
  });
  ...
  function checkPendings() {
    $.ajax({
      url: '/go/myresources_v2/checkHMPendings',
      type: 'GET',
      success: function (response) {
        if ($.trim(response) != '') {
          displayHMPendingNotification(response);
        }
      },
  ...
  $.ajax({
      ...
      if (response.titles !== '') {
        ...
        updateHotmathsUrl(); // ANZGO-3897 added by jdchavez 10/19/2018
      } else {
  ...
  function updateHotmathsUrl () {
    $('a[href*="/go/myresources_v2/toHotmaths/"]').each(function (index, el) {
