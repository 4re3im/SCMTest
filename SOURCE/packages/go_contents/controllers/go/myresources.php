...
Loader::model('cup_go_external_user', 'cup_go');
...
class GoMyResourcesController extends Controller
{
    const TITLES            = 'titles';
    ...
    public function on_start()
    {
        ...
        // SB-419 updated by mabrigos add version number
        ...
        $this->addFooterItem(
            ...
            (string)$html->javascript('myresources.js', $this->pkgHandle)->href . '?v=14.2"></script>'
        );
        // ANZGO-3227
    ...
    public function view($arrange = null)
    {
        ...
        // SB-419 added by mabrigos to get total number of susbcriptions
        $activationList = new HubActivationList();
        $activationList->fetchMyResourcesList($u->getUserID());
        $this->set('totalSubscriptions', count($activationList->subscriptions));
        ...
        foreach ($subscriptions as $titleId => $tabsSubscribed) {
            ...
            foreach ($tabsSubscribed as $tabSubscriptionData) {
                ...
                if ($source === 'HOTMATHS') {
                    ...
                    // if $tabEndDate is null, then access Edjin API to get an expiry date
                    if (is_null($tabEndDate)) {
                        $tabEndDate = $this->checkAndUpdateBrandExpiryDate($externalID);
                    }

                    // SB-9 added by jbernardez 20191106
                    $isExpired = false;

                    if (strtotime($tabEndDate) <= strtotime(date('d-m-Y'))) {
                        $isExpired = true;
                    }

                    // SB-9 added by jbernardez 20191105
                    // SB-424 modified by jbernardez 20200131
                    $tiles[$tabID] = array(
                        'name'          => $displayName,
                        'tileIcon'      => $image,
                        'endDate'       => date('d-m-Y', strtotime($tabEndDate)),
     ...
     * check product expiration date on edjin
     * save value to cupGoExternalUser.brandExpiryDate
     * @param $uid
     * @return bool
     */
    private function checkAndUpdateBrandExpiryDate($externalId)
    {
        global $u;
        $params = array(
            'userId' => $u->uID,
        ...
        if ($api !== false) {
            $resHMUser = $api->getHmUserExternalID();
            $endDate = $resHMUser->products[0]->endDate / 1000;

            $dt = new DateTime("@$endDate");  // convert UNIX timestamp to PHP DateTime
            $brandExpiryDate = $dt->format('Y-m-d H:i:s'); // output = 2017-01-01 00:00:00

            if ($brandExpiryDate) {
                $hmAPI = new CupGoExternalUser();
                $hmAPI->updateBrandExpiryDate($brandExpiryDate, $externalId);

                return $brandExpiryDate;
            }
