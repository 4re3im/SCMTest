...
class Concrete5_Helper_Form {
	...
	protected function inputType($key, $type, $valueOrArray, $miscFields) {
		...
		return "<input id=\"$key\" type=\"$type\" name=\"$key\" value=\"$val\" " . $this->parseMiscFields("go-input", $miscFields) . ' />';
	}
	...
	*/
	...
	public function select($key, $optionValues, $valueOrArray = false, $miscFields = array()) {
		...
		$str = '<select name="' . $key . '" id="' . $id . '" ' . $this->parseMiscFields('form-control go-input', $miscFields) . '>';
