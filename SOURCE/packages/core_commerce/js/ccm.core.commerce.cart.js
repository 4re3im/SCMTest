...
/* Added for ticket 1390. 
Handles the hiding of the "Next" button during credit card payment form submission. 
If the "next" button has been clicked once, it will hide the button and show a loading gif then it will submit the form.
After submission, the form structure will be again reset.
Note: This function is being called twice due to unknown reasons.
*/
...
$(document).on("click","#submit_next",function(){
	if(!nextClicked) {
		...
		// Check if we are in the correct page or path.
		if(currentPath.indexOf("payment/form") > 0) {
			// If in the correct URL, hide button.
			$(this).hide();
			var buttonContainer = $(".ccm-core-commerce-cart-buttons");
			var loadingImage = buttonContainer.find("img");
			// Since this function is being called twice, it may generate two loading images, so check that as well.
			// If there is no loading image yet, append an image.
			if(loadingImage.length < 1) {
				buttonContainer.append("<img src='../../updates/concrete5.6.3.1_updater/concrete/images/throbber_white_16.gif' width='16' height='16' style='float:right;'/>");
			}
		}
		// Return state of the "nextClicked".
		nextClicked = false;
	}
});	
