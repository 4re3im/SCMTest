...
class DashboardCoreCommerceDiscountsController extends Controller {
	...
	public function add() {
		...
		if ($e->has()) {
			...
			$this->setUsersAndGroups($ed);
			$this->redirect('/dashboard/core_commerce/discounts/', 'discount_created');
		}
	}
	
	protected function setUsersAndGroups($discount) {
		$discount->setDiscountGroups($this->post('discountFilterGroups'));
		$discount->setDiscountUsers($this->post('discountFilterUsers'));
	}
	...
	public function edit($discountID = 0) {
		...
		if ($this->isPost()) {
			...
			if ($e->has()) {
				...
				$this->setUsersAndGroups($discount);
