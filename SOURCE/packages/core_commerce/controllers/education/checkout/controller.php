...
class CheckoutController extends Controller {
	...
	public function getCheckoutNextStepButton($text=false) {
		return $this->checkoutStepHelper->getCheckoutNextStepButton($text);
	}
	...
	public function on_start() {
		...
		$this->addHeaderItem(Loader::helper('html')->css('ccm.core.commerce.singlepage.checkout.css', 'core_commerce'));
		...
		if($pkg->config('ONE_PAGE_CHECKOUT')) {
			$this->testStep = false;
			$this->onePage = true;
			$this->addHeaderItem(Loader::helper('html')->javascript('ccm.core.commerce.singlepage.checkout.js', 'core_commerce'));
		} else {
		...
		// ommit the order attributes that are manually displayed
		$akHandles = array('billing_first_name', 'billing_last_name', 'billing_address', 'billing_phone', 'shipping_first_name', 'shipping_last_name', 'shipping_address', 'shipping_phone');
