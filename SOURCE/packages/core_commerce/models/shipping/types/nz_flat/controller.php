...
class CoreCommerceNzFlatShippingTypeController extends CoreCommerceShippingController {

	protected $shippingMethods = array(
		'NZ_FLAT' => 'Postage & Handling'
	);
	...
	public function type_form() {
		...
		$this->set('SHIPPING_TYPE_NZ_FLAT_BASE', $SHIPPING_TYPE_FLAT_BASE);
		$this->set('SHIPPING_TYPE_NZ_FLAT_PER_ITEM', $SHIPPING_TYPE_FLAT_PER_ITEM);
	}
	...
	public function save() {
		$pkg = Package::getByHandle('core_commerce');
		$pkg->saveConfig('SHIPPING_TYPE_NZ_FLAT_BASE', $this->post('SHIPPING_TYPE_FLAT_BASE'));
		$pkg->saveConfig('SHIPPING_TYPE_NZ_FLAT_PER_ITEM', $this->post('SHIPPING_TYPE_FLAT_PER_ITEM'));
	}
	
	public function getAvailableShippingMethods($currentOrder) {
		$pkg = Package::getByHandle('core_commerce');
		$shipping = $pkg->config('SHIPPING_TYPE_NZ_FLAT_BASE');
		$perItem = $pkg->config('SHIPPING_TYPE_NZ_FLAT_PER_ITEM');
		...
		$ecm = new CoreCommerceShippingMethod($this->getShippingType(), 'NZ_FLAT');
