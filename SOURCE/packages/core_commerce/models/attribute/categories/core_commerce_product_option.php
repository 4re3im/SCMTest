...
class CoreCommerceProductOptionAttributeKey extends AttributeKey {
	...
	public function assignGlobalKeyToProduct($product) {
		$db = Loader::db();
		$db->Replace('CoreCommerceProductOptionAttributeKeys', array('productID' => $product->getProductID(), 'akID' => $this->getAttributeKeyID(), 'poakIsRequired' => $this->isProductOptionAttributeKeyRequired()), array('akID', 'productID'));
	}
	
	public function removeGlobalKeyFromProduct($product) {
		$db = Loader::db();
		$db->Execute('delete from CoreCommerceProductOptionAttributeKeys where akID = ? and productID = ?', array($this->getAttributeKeyID(), $product->getProductID()));
	}
