...
class CupContentTitleList extends DatabaseItemList
{
    ...
    //'CoreCommerceProductAttributeKey';
    ...
    public function filterBySubject($subjectName)
    {
        $db = Loader::db();
        $subjectName = $db->quote($subjectName);
        $sub_query = '( SELECT cts.titleID FROM CupContentTitleSubjects cts WHERE cts.subject like '. $subjectName.')';
    ...
    public function filterByGoTitle()
    {
        $this->filter('ti.isGoTitle', 1, '=');
    }
}
