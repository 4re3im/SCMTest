...
$url = $th->getToolsURL('search_trials', 'cup_content');
...
<div class="span16">
	...
	<div class="clearfix">
		<?php echo $form->label('trialID', t('Trial ID'))?>
		<div class="input">
			<input type="text" class="form-control span6" id="search-entitlement" search-url="<?php echo $url ?>"
				placeholder="Type in the Subscription's Name/ISBN and select an entitlement."><br />
			<?php echo $form->text(
				'trialID',
				@$entry['trialID'],
				array('class' => "span6", 'style' => "pointer-events: none;background-color:#E9ECEF")
			)?>
		</div>
	</div>
