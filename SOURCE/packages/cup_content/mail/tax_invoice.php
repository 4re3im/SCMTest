...
//$subject = "Cambridge University Press Tax Invoice: ".$email_data['invoiceNumber'];
$subject = "Cambridge University Press Tax Invoice: ".$email_data['orderID'];


ob_start();
include(dirname(__FILE__).'/invoice.html');
$body_html_tpl = ob_get_clean();

ob_start();
include(dirname(__FILE__).'/invoice_item.html');
...
foreach($email_data['products'] as $item){
	$repace_values = array(
							'%%INVOICE_NUMBER%%' => $email_data['orderID'], //$email_data['invoiceNumber'],
...
$repace_values = array(
							'%%INVOICE_NUMBER%%' => $email_data['orderID'], //$email_data['invoiceNumber'],
							'%%ORDER_DATE%%' => date('d/M/Y', strtotime($email_data['orderDate'])),
