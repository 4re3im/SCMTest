...
var cup_carousel_start = function(json_config_url){
	jQuery('.btn_department_frame').each(function(){
	...
	var indicator_block = jQuery('.cup-main-carousel .loading_indicator');
	...
	var carousel_frame = jQuery('.cup-main-carousel .carousel_frame');
	var unit_width = carousel_frame.width();
	var image_holder = jQuery('.cup-main-carousel .carousel_frame .image_holder');
	var image_frame = jQuery('.cup-main-carousel .carousel_frame .image_frame');
	...
	var description_frame = jQuery('.cup-main-carousel .carousel_frame .description_frame');
	var info_frame = jQuery('.cup-main-carousel .carousel_frame .description_frame .info_frame');
	var content_area = info_frame.find('.content_area');
	var indicator_holder = jQuery('.cup-main-carousel .carousel_frame .description_frame .indication_frame');
	...
	var showSlide = function(selected_idx){
			...
			var tmp_title = jQuery('<h5></h5>');
				tmp_title.text(carousle_data[selected_idx]['title']);
			var tmp_description = jQuery('<p></p>');
				tmp_description.text(carousle_data[selected_idx]['description']);
			var tmp_link = carousle_data[selected_idx]['link']
			
			description_frame.show();
			//carousel_frame.css({background:'url('+image_src+') no-repeat'});
			/*
			var last_image = image_frame.find('.image_layer.active');
			var next_image = image_frame.find('.image_layer').eq(selected_idx);
			
			last_image.removeClass('active');
			last_image.hide('slide', {direction:'left'},  200);
			
			next_image.addClass('active');
			next_image.delay(200).show('slide', {direction:'right'},  200);
			*/
			...
			var alink = jQuery('.cup-main-carousel .carousel_frame .description_frame .btn_frame a');
            alink.attr('href', tmp_link);
            //alink.attr('href', "javascript:void;");

            var region = "AU";
            if(getCookie("DEFAULT_LOCALE") == "en_NZ"){
                region = "NZ";
            }
            var tracking_line = region + "-" + selected_idx + "-"+ tmp_title.text();
            alink.click(function(){
                //alert("title:"+tracking_line);
                _gaq.push(['_trackEvent', 'Home Carousel', 'Click', tracking_line]);
	...
	jQuery.getJSON(json_config_url, function(json){
		...
		carousle_data = json.carousel_data;
		
		for(var i=0; i < carousle_data.length; i++){
			image_array.push(carousle_data[i]['image']);
		}
		...
		$.imgpreload(image_array,{
			all: function()
			{
				for(var idx = 0; idx < carousle_data.length; idx++){
					//var img = jQuery('<img/>').attr('src', carousle_data[idx]['image']).css('display','none');
					var img_layer = jQuery('<div></div').addClass('image_layer');
						img_layer.css('background', "url("+carousle_data[idx]['image']+") no-repeat");
						img_layer.hide();
					var img = jQuery('<img/>').attr('src', carousle_data[idx]['image']);
					...
					indicator.click(function(){
						...
						loop_idx = jQuery('.cup-main-carousel .carousel_frame .indicator').index(this);
						showSlide(loop_idx);
					});
