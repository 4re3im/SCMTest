		...
		<tbody>
			<?php foreach($events as $idx=>$event):?>
				<tr ref="<?php echo $event->id;?>">
					...
					<td><?php echo $event->getEntryNumber('pending');?></td>
					<td><?php echo $event->getEntryNumber();?></td>
					<td><?php echo date('Y-m-d H:i:s', strtotime($event->createdAt));?></td>
					<td>
						<a href="<?php echo View::url('/dashboard/cup_competition/entry/viewByEvent', $event->id);?>">View Entries</a>
					</td>
				</tr>
			<?php endforeach;?>
		</tbody>
