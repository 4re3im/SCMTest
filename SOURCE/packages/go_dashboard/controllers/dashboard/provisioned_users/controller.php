...
/**
 * Provisioned Users Block Controller for Booksellers
 * ANZGO-3595 Added by Shane Camus 01/25/2018
 */

defined('C5_EXECUTE') || die(_("Access Denied."));

// HUB-127 Added by Carl Lewi Godoy, 08/14/2018
...
class DashboardProvisionedUsersController extends Controller
{
    private $pkgHandle = 'go_dashboard';

    protected $hasStillActiveSubscription;

    public function on_start()
    {
        Loader::library('HotMaths/api');
        Loader::model('provisioned_users/model', $this->pkgHandle);
        Loader::model('provisioned_users/list', $this->pkgHandle);
    ...
    {
        $u = new User();
        $provisionedUsersList = new GoDashboardProvisionedUsersList();
        $provisionedUsersList->filterByStaffID($u->getUserID());
        $provisionedUsersList->sortByCreatedDate();
        $this->set('provisionedUsersList', $provisionedUsersList->getPage());
        $this->set('pagination', $provisionedUsersList->displayPaging(false, true));
    }
    ...
    {
        $postID = filter_input(INPUT_POST, 'userID');
        if ($postID) {
            $id = $postID;
        }
        ...
        $this->set('form', $form);
        $this->addHeaderItem($html->css('go_dashboard_user.css', $this->pkgHandle));
        $this->addHeaderItem($html->javascript('go_dashboard_user.js', $this->pkgHandle));
        $this->addHeaderItem($html->css('user_subscription.css', $this->pkgHandle));

        $jsPath = (string)$html->javascript('user_subscription.js', $this->pkgHandle)->file . "?v=1";
        $this->addFooterItem('<script type="text/javascript" src="' . $jsPath . '"></script>');

        $userList = new GoDashboardProvisionedUsersList();
        $userObj = new GoDashboardProvisionedUsers();
        ...
        if (isset($id) && !empty($id)) {
            ...
            // HUB-127 Added by Carl Lewi Godoy, 08/15/2018
            ...
            if ($this->post(SEARCH_STRING) != '') {
                $userList->filterByuID($this->post(SEARCH_STRING));
                $noteResults = $userObj->getUserNotes($this->post(SEARCH_STRING));
                $userSubscriptions = Activation::where([
                    'user_id' => $this->post(SEARCH_STRING),
                    'orderField' => 'created_at',
                    'orderDirection' => 'DESC',
                    'is_paginated' => 0
                ]);
                $userTrackingGeneral = $userObj->getUserTrackingGeneral($this->post(SEARCH_STRING));
                $userActivationErrors = $userObj->getUserActivationErrors($this->post(SEARCH_STRING));
            } else {
                $this->redirect('dashboard/provisioned_users');
            }
        }
    ...
    public function getUserTrackingGeneral()
    {
        ...
        $userTrackingGeneral = new GoDashboardProvisionedUsers();
        $userTrackingGeneralResult = $userTrackingGeneral->getUserTrackingGeneral($userID);
        ...
        if (count($userTrackingGeneralResult) > 0) {
            ...
            echo "Sorry, Your query Returned empty result, please try again!";
            echo "</div>";
        }
        exit;
    }
    ...
    public function addNote()
    {
        ...
        $userNotes = new GoDashboardProvisionedUsers();
    ...
    public function toggleUserSubscription()
    {
        ...
        // HUB-128 Added by Carl Lewi Godoy 08/15/2018
    ...
    public function displayUserSubscription($userID = null)
    {
        $userSubscriptions = [];
        if ($userID !== null ){
            $userSubscriptions = Activation::where([
    ...
    public function saveUserGeneralInfo()
    {
        ...
        $dashboardGoUsers = new GoDashboardProvisionedUsers($id);

        $dashboardGoUsers->saveUserGeneralInfo($data);

        if ($manuallyActivatedChecker) {
            $userInfo = $u->getUserInfo($id);
            $staff = new GoDashboardProvisionedUsers($userInfo['ak_uMAStaffID']);
    ...
    public function generateResetPasswordLink()
    {
        ...
        if (!$u->isLoggedIn() || !array_intersect($u->getUserGroups(), ["Administrators", "CUP Staff", "Bookseller"])) {
    ...
    public function createSubscriptionHtml($userId)
    {
        ...
        foreach ($activations as $activation) {
            if ($activation->CreatedBy !== $createdBy) {
                continue;
            }
        ...
        $args = array('subscriptions' => $newSubs, 'showCreator' => false);
        ob_start();
