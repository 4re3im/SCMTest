...
class ProvisioningModel
{
    ...
    public function updateProvisionedUsersStatus($id, $status)
    {
        $sql = 'UPDATE ProvisioningUsers SET Status = ?, DateModified = NOW() WHERE id = ?';

        return $this->db->Execute($sql, array($status, $id));
    }
...
SQL;
    ...
    public function processUserSubscription($provisioningUser, $saIDs, $limitedEndDate = null)
    {
        ...
        foreach ($saIDs as $saID) {
            ...
            if ($hasNoError && !$result['success'] && ($result['remainingRetryCount'] === 0)) {
                $this->updateProvisioningUserStatusByID($provisioningUser['ID'], $result['action'], $result['message']);
                $hasNoError = false;
            }

            if ($result['message'] === 'Failed adding subscription') {
                $subscription['countFailedSubscription']=+1;
            }
        ...
        $subscription['hasError'] = !$hasNoError;

        if (!empty($hmMetaData)) {
            $subscription['products'] = $hmMetaData;
        }
