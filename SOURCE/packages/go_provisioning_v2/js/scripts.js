...
var successProvision = function (data) {
  ...
  showSubscriptionStats(data.table);
  // finishProvisioning();
};
...
function updateProgressBar () {
  completed++;
  progress = Math.round((completed / (total + GIGYA_PROCESS)) * 100);
  percentage = progress + '%';
  var progressBar = $('#provisioning-progress');
  $(progressBar).find('.bar').animate({ width: percentage }).text(percentage);
}
...
function postSubmitForm (data) {
  ...
  for (var i = 0; i < data.total; i += data.pagination) {
    addAjax({ 
      url: '/dashboard/provisioning_v2/setup/provision/' + data.fileRecordId + '/' + i,
      type: 'POST',
      dataType: 'json',
      data: postData,
    ...
    // $.ajax({
    //   url: '/dashboard/provisioning_v2/setup/provision/' + data.fileRecordId,
    //   type: 'POST',
    //   dataType: 'json',
    //   data: postData,
    //   success: successProvision,
...
function provisionInGigya () {
  ...
  $.ajax({
    url: '/dashboard/provisioning_v2/setup/provisionInGigya/' + fileRecordId + '/' + providerId,
    type: 'POST',
    dataType: 'json',
    beforeSend : function(){
...
function checkJobStatus (scheduleId) {
  $.ajax({
    url : '/dashboard/provisioning_v2/setup/getJobStatus/' + scheduleId,
    type : 'GET',
    dataType: 'json',
    beforeSend : function() {
...
function checkUsersInGigya () {
  ...
  var providerId = $('#providerId').val();
  $.ajax({
    url: '/dashboard/provisioning_v2/setup/updateGigyaStatus/' + fileRecordId + '/' + providerId,
    type: 'POST',
    dataType: 'json',
    beforeSend: function() {
      showStatusInfo('Checking the users\' Gigya migration status. Please wait.');
    },
    success: function (data) {
      ...
      updateResultTable(data, true);
      showStats(data);
      // autoVerifyUsersInGigya(data.gUIds);
    },
    error : function() {
...
function autoVerifyUsersInGigya (gUIds) {
  // GCAP-541 added by machua/mtanada 20191015
  var providerId = $('#providerId').val();
  $.ajax({
    url: '/dashboard/provisioning_v2/setup/autoVerifyUsersInGigya/' + fileRecordId + '/' + providerId,
    type: 'POST',
    data: {gUIds: gUIds},
    dataType: 'json',
    beforeSend: function() {
      showStatusInfo('Tagging users\' account as verified in Gigya. Please wait.');
    },
    success: function (data) {
      showStatusInfo('Users\' account is successfully tagged as verified.');
      updateProgressBar();
      updateResultTable(data, true);
      showStats(data);
    },
    error : function() {
      showStatusInfo('Unable to tag the users as verified.');
    },
    complete : function(){
      console.log('Gigya time end: ' + new Date());
      hideInfoShowControls();
...
function checkGigyaProvisionStatus () {
  $.ajax({
    url: '/dashboard/provisioning_v2/setup/getGigyaProvisioningStatus/' + fileRecordId,
    type: 'POST',
    dataType: 'json',
    success: function (data) {
      if (!data.isDone) {
...
function refreshResult (fileRecordId, successCallback) {
  $.ajax({
    url: '/dashboard/provisioning_v2/setup/displayUsers/' + 1 + '/' + fileRecordId,
    type: 'POST',
    dataType: 'json',
...
function showSubscriptionStats (data) {
  var failedSubscription = data.totalFailedSubscription;
  $('#failed-subscription').html(failedSubscription);
  $('#subscription-stats-table').show();
}
