...
class BulkDeleteModel extends Model
{
    private $db;
    const FILE_TABLE = 'BulkDeleteFiles';
    const USER_TABLE = 'BulkDeleteUsers';
    ...
    {
        $sql = 'SELECT FirstName, LastName, Email, Status, FileID, DateUploaded FROM ' . static::USER_TABLE;
        $sql .= ' WHERE FileID = ?';
        return $this->db->GetAll($sql, [$fileID]);
    }
    ...
    public function updateSuccessfulChangedUsers($emails, $fileID, $data)
    {
    ...
    {
        $query = 'DELETE FROM ' . static::USER_TABLE;
        $query .= ' WHERE DATE(DateUploaded) <= DATE_SUB(CURDATE(), INTERVAL 7 DAY)';
